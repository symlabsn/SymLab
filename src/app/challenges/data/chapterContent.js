...
- Experimenter avec du code
- Visualiser des resultats immediatement
- Documenter votre travail avec du texte et des formules mathematiques
- Partager vos analyses

## Installation avec Anaconda

**Anaconda** est une distribution Python qui inclut Jupyter et plus de 250 packages scientifiques pre-installes (NumPy, Sympy, Matplotlib, etc.).

### Etapes d'installation :

1. Rendez-vous sur [anaconda.com/download](https://anaconda.com/download)
2. Telecharger la version pour votre systeme (Windows, Mac, Linux)
3. Executez l'installateur et suivez les instructions
4. Une fois installe, lancez "Anaconda Navigator"
...
# Les resultats s'affichent directement sous la cellule
resultat = 2 + 2
...
- **Gratuit** : Acces a des GPU/TPU pour le calcul intensif
- **Collaboratif** : Partagez vos notebooks comme des Google Docs
- **Pre-configure** : NumPy, Sympy, Matplotlib deja installes
...
- **Cellules de code** : Pour ecrire et executer du Python
...
# Verification des packages scientifiques
import numpy as np
import sympy as sp
...
3. Dans la premiere cellule, tapez :
   \`\`\`python
   import sympy as sp
   sp.init_printing()
   \`\`\`
...
\`\`\`python
dir(str)     # Liste toutes les methodes d'un objet
\`\`\`
...
# 2. Fonction dir() - Liste des attributs/methodes
print("\\n=== Methodes du type 'list' ===")
methodes = [m for m in dir(list) if not m.startswith('_')]
print(methodes[:10], "...")  # 10 premieres
