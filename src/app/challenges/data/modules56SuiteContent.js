...
plt.show()`,
        ...
        polynomials_visual: {
                ...
                theorie: `
## Visualiser les polynômes

Les racines d'un polynôme $P(x)$ sont les points où la courbe coupe l'axe des abscisses ($y=0$).

### Comportement
- **Degré pair** : Tend vers $+\\infty$ ou $-\\infty$ des deux côtés.
- **Degré impair** : Tend vers $-\\infty$ d'un côté et $+\\infty$ de l'autre.
        `,
                code: `import numpy as np
import matplotlib.pyplot as plt

# Définir le polynôme : P(x) = (x-2)(x+1)(x-1) = x^3 - 2x^2 - x + 2
def P(x):
    return (x - 2) * (x + 1) * (x - 1)
...
plt.plot(x, y, linewidth=2, label='$P(x) = (x-2)(x+1)(x-1)$')

# Marquer les racines
racines = [-1, 1, 2]
plt.plot(racines, [0, 0, 0], 'ro', markersize=10, label='Racines')

plt.axhline(0, color='black', linewidth=1)
plt.axvline(0, color='black', linewidth=1)
plt.title("Visualisation des racines d'un polynôme")
plt.grid(True, alpha=0.3)
plt.legend()
plt.show()`,
                exercice: `Tracez le polynôme $P(x) = x^4 - 4x^2$ et trouvez ses 3 racines.`
        }
};
